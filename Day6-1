...
if($_POST){
   ...
   $area = $_POST['txt7'];
    ...
    $q =mysqli_query($connection, "insert into s_register (st_id,st_name,st_gender,st_mobile,st_college,st_city,st_area,st_pincode,st_password,st_birthdate) values ('{$id}','{$name}','{$gender}','{$mobile}','{$college}','{$city}','{$area}','{$pin}','{$password}','{$dob}')") or die(mysqli_error($connection));
if ($q){
...
<html>
    <body>
        ...
        <tr><td>Area : </td><td><select name="txt7">
         ...
         <tr><td>Pin code : </td><td><input type="number" name="txt8"/><br/></td></tr>
        <tr><td>Password : </td><td><input type="password" name="txt9"/><br/></td></tr>
